...
namespace LunarLander.Screens
{
    public class Pause : GameScreen
    {
        ...
        {
            if(input.KeyboardState.WasKeyPressed(Keys.Tab) ||           //Pause to Main
                input.WasPressed(0, InputHandler.ButtonType.Y, Keys.Y))
            {
                screen.Pop();
            }

            if (input.KeyboardState.WasKeyPressed(Keys.Escape) ||       //Pause to Exit
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            Vector2 v = spriteFont.MeasureString("Pause") / new Vector2(2,2);

            spriteBatch.DrawString(spriteFont, "Pause", (center - v), Color.White);
